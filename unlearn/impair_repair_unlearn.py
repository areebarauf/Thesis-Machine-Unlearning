...
class ImpairRepair:
    ...
    def generate_noise_for_retain_classes(self):
        for cls in self.classes_to_retain:
            print("Optimizing loss for retain class {}".format(cls))
            self.noises[cls] = Noise(self.batch_size, 3, 32, 32).cuda()
            opt = torch.optim.Adam(self.noises[cls].parameters(), lr=0.1)
    ...
    def impair(self):
        ...
        print(f"Impairment loss: {running_loss / (len(self.classes_to_forget) + len(self.classes_to_retain) * 20 * self.batch_size)}, Impairment Acc: {running_acc * 100 / (len(self.classes_to_forget) + len(self.classes_to_retain) * 20 * self.batch_size)}%")
    ...
    def repair(self):
        ...
        for cls in self.classes_to_retain:
            ...
            for i, data in enumerate(noisy_repair_loader):
                ...
                assert out.shape == labels.shape
                running_acc += (labels == out).sum().item()
        print(f"Repair loss: {running_loss / (len(self.classes_to_retain) * 20 * self.batch_size)}, Repair Acc: {running_acc * 100 / (len(self.classes_to_retain) * 20 * self.batch_size)}%")
